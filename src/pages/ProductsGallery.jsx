  ...
  return (
    <div ref={dropdownRef} className={`relative ${className}`}>
      <button
        ...
        className={`
          w-full border border-transparent border-primary cursor-pointer rounded-md px-3 py-2 text-sm text-left 
...
export default function ProductPage() {
  ...
  const fetchAllProducts = async () => {
    ...
    try {
      setLoading(true);
      const response = await instance.get(`/products/all-products?limit=10000`);
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1)
                  ...
                  .map((page) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg border cursor-pointer transition-all font-bold ${
                        currentPage === page
                          ? "bg-[#80011f] text-[#ffdf9e] "
