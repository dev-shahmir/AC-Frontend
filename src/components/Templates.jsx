...
export default function Templates() {
  ...
  const fetchTemplates = async () => {
    try {
      const response = await instance.get(
        "/custom-products/custom-admin-product-templates?limit=8"
      );
  ...
  useEffect(() => {
    fetchTemplates();
    const interval = setInterval(fetchTemplates, 1 * 60 * 1000);
  ...
  // Function to check if product is new (within 30 days)
  ...
  // Infinite slider navigation functions
  const nextSlide = () => {
    setCurrentIndex(
      (prev) => (prev + 1) % products.length // 👈 infinite loop
    );
  };

  const prevSlide = () => {
    setCurrentIndex(
      (prev) => (prev - 1 + products.length) % products.length // 👈 infinite loop
    );
  };
  ...
  const ProductCard = ({ product, index, isSlider = false }) => {
    const firstSize =
    ...
    return (
      <div
        key={product._id}
        className={`group cursor-pointer ${isSlider ? "flex-shrink-0" : ""}`}
        onClick={() => navigate(`/customize-product/${product._id}`)}
        style={{
          animationDelay: `${index * 1000}ms`,
          width: isSlider ? `${100 / products.length}%` : "auto",
        }}
      >
        <div className="relative rounded-xl md:rounded-2xl overflow-hidden transition-all duration-500 transform hover:-translate-y-1 border border-[#ffefcc]">
          ...
          <div className="relative aspect-square overflow-hidden">
            ...
            {/* Badges - Show only if product is new */}
            ...
            <div className="absolute bottom-1 left-1 right-1 md:bottom-4 md:left-4 md:right-4 hidden md:block">
              <button className="quick-view-btn poppins-medium cursor-pointer w-full bg-transparent backdrop-blur-sm text-[#ffdf9e] py-2 md:py-3 px-2 md:px-4 rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all duration-300 border border-[#ffefcc] opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0">
  ...
  return (
    <section className="bg-[#ffefcc] pb-4 md:pb-6 lg:pb-10">
        ...
        <div className="text-center mb-6 md:mb-10">
          <div className="animate-fade-in">
            ...
            <p className="text-[#80011f] poppins-regular text-lg pt-2 max-w-2xl mx-auto">
              Discover our latest customized creations, tailored to bring style,
              elegance, and individuality into your space.
            </p>
          </div>
          ...
          <div className="flex items-center justify-center gap-4 mt-6">
            <button
              ...
              className={`px-4 py-2 rounded-lg transition-all poppins-regular duration-300 cursor-pointer ${
                viewMode === "grid"
                  ? "bg-[#80011f] text-[#ffefcc]"
                  : "bg-transparent text-[#80011f] border border-[#80011f]"
              }`}
            >
              Grid View
            </button>
        ...
        {loading && (
          <div
            className={
              viewMode === "slider"
                ? "flex gap-6"
                : "grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6 lg:gap-8"
            }
          >
            ...
            {viewMode === "slider" ? (
              <div className="relative">
                ...
                <div className="flex items-center justify-between mb-6">
                  <div className="text-sm poppins-light text-[#80011f]/70">
                    Showing {((currentIndex % products.length) + 1)}-
                   {Math.min(((currentIndex % products.length) + itemsToShow), products.length)} of
                    total products
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={prevSlide}
                     className="p-2 rounded-full cursor-pointer transition-colors duration-300 bg-[#80011f] text-[#ffefcc] hover:bg-[#a0021f]"
                    >
                ...
                <div className="relative overflow-hidden rounded-2xl">
                  <div
                    className="flex transition-transform duration-500 ease-in-out gap-6 mt-4"
                    style={{
                      transform: `translateX(-${(currentIndex * 100) / itemsToShow}%)`,
                      width: `${(products.length * 100) / itemsToShow}%`
                    }}
                  >
                     {getExtendedProducts().map((product, index) => (
                      <ProductCard
        ...
        {!loading && products.length > 0 && (
          <div className="text-center mt-12 md:mt-16">
            <button
              ...
              className="btn-md btn items-center poppins-regular gap-2 bg-transparent text-[#80011f] border border-[#80011f] textprimary cursor-pointer rounded-xl font-semibold text-lg px-6 py-3 inline-flex transition-all duration-300 hover:bg-[#80011f] hover:text-[#ffefcc] hover:shadow-lg hover:scale-105"
            >
              View Customize Products
              <svg
